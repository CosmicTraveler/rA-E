...
static void boardcast_refine_message(map_session_data* sd, struct item& item, bool is_success)
{
	...
	if(is_success && item.refine >= battle_config.refine_succes_announce){
		sprintf(msg,msg_txt(sd,1601),sd->status.name,item_db.create_item_link(item).c_str());
		...
		sprintf(msg,msg_txt(sd,1602),sd->status.name,item_db.create_item_link(item).c_str());
